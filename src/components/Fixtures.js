  ...
  useEffect(() => {
    ...
    const getFixtures = async () => {
      try {
        ...
        // Current date needs to have a 0 at the beginning if the date is between 1 - 9
        if (currentDate >= 1 && currentDate <= 9) {
          currentDate = `0${currentDate}`
        }
        if (currentMonth < 7) {
          fullDateString = `${currentYear}-0${currentMonth + 1}-${currentDate}`
          const { data: { matches } } = await authenticated.get(`https://api.football-data.org/v4/competitions/2021/matches?season=${currentYear - 1}&dateTo=${fullDateString}&dateFrom=${fullDateString}`)
          setFixtures(matches)
        } else if (currentMonth >= 7) {
          if (currentMonth >= 9) {
            fullDateString = `${currentYear}-${currentMonth + 1}-${currentDate}`
            const { data: { matches } } = await authenticated.get(`https://api.football-data.org/v4/competitions/2021/matches?season=${currentYear}&dateTo=${fullDateString}&dateFrom=${fullDateString}`)
            setFixtures(matches)
          } else {
            fullDateString = `${currentYear}-0${currentMonth + 1}-${currentDate}`
            const { data: { matches } } = await authenticated.get(`https://api.football-data.org/v4/competitions/2021/matches?season=${currentYear}&dateTo=${fullDateString}&dateFrom=${fullDateString}`)
